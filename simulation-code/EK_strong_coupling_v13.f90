...
program Eguchi_Kawai
  ...
  open(unit=10,status='OLD',file='input_v13.dat',action='READ')
...
SUBROUTINE kinetic(kin_total,U,alpha,NMAT,NDIM,NSite,LatticeSpacing) 
  ...
  integer t,tp1,imat,jmat,idim,NMAT,NSite,NDIM
  doublecomplex U(1:NSite,1:NDIM,1:NMAT,1:NMAT)
  double precision alpha(1:nmat)
  double complex expialpha(1:nmat)
  
  doubleprecision kin,kin_total,LatticeSpacing
  ...
  do t=1,nsite
     ...
     do idim=1,ndim
        ...
        do imat=1,NMAT
           do jmat=1,NMAT
              kin=kin+dble(expialpha(imat)*U(tp1,idim,imat,jmat)&
                   *dconjg(expialpha(jmat))*dconjg(U(t,idim,imat,jmat)))
           end do
        end do
        kin=dble(NMAT)-kin
        kin=kin/LatticeSpacing*dble(NMAT)
        kin_total=kin_total+kin
     end do
  end do
...
SUBROUTINE Calc_Wilson(U,NMAT,NDIM,NSite,wilson) 
  ...
  integer t, imat,idim
  doublecomplex U(1:NSite,1:NDIM,1:NMAT,1:NMAT),temp
  ...
  do t=1,nsite
     do idim=1,ndim
        temp=(0d0,0d0)
        do imat=1,nmat
           temp=temp+U(t,idim,imat,imat)
        end do
        wilson=wilson+abs(temp)
     end do
  end do
...
SUBROUTINE MolecularEvolution(NMAT,NDIM,NSite,Ntau,Dtau_U,Dtau_alpha,U,alpha,&
  ...
  integer t,tm1,l,p,q,r,NSite,Ntau,NMAT,acceptance,trial,ite,NDIM,idim,ndec
  ...
  do t=1,NSite
     do idim=1,ndim
        do imat=1,nmat
           do jmat=1,nmat
              MAT(imat,jmat)=P_U(t,idim,imat,jmat)*dcmplx(0.5d0*Dtau_U)
           end do
        end do
        call MATRIX_iEXP(NMAT,MAT,MATiEXP)
        do imat=1,NMAT
           do jmat=1,NMAT
              U_Old(imat,jmat)=U(t,idim,imat,jmat)
        ...
        do imat=1,NMAT
           do jmat=1,NMAT
              do kmat=1,NMAT
                 U(t,idim,imat,jmat)=U(t,idim,imat,jmat)+MATiEXP(imat,kmat)*U_Old(kmat,jmat) 
              end do
           end do
        end do
     end do
  ...
  do ite=2,Ntau
     ...
     do t=1,NSite
        do idim=1,ndim
           do imat=1,nmat
              do jmat=1,nmat
                 MAT(imat,jmat)=P_U(t,idim,imat,jmat)*dcmplx(Dtau_U)
              end do
           end do
           call MATRIX_iEXP(NMAT,MAT,MATiEXP)
           do imat=1,NMAT
...
SUBROUTINE MakeDerivHamiltonian_alpha(U,alpha,NMAT,DIM,NumSite,DerivH,LatticeSpacing,g_coeff,P_fix,P_fix_width,ndec,g_coeff_alpha)
  ...
  integer t,tp1,l,p,NumSite,NMAT,DIM,it,imat,jmat,kmat,ndec,imax,imin
        ...
        do imat=1,NMAT
           do jmat=1,NMAT
              temp(imat,jmat)=temp(imat,jmat)&
                   +U(tp1,p,imat,jmat)*dconjg(U(t,p,imat,jmat))&
...
SUBROUTINE MakeDerivHamiltonian_U(U,V,NMAT,NDIM,NSite,DerivH,LatticeSpacing)
  ...
  integer t,tp1,tm1,l,q,r,NSite,NMAT,NDIM,idim,jdim,imat,jmat,kmat
  
  doublecomplex U(1:NSite,1:NDIM,1:NMAT,1:NMAT)
  doublecomplex V(1:NMAT)
  doubleprecision LatticeSpacing
  doublecomplex DerivH(1:nsite,1:ndim,1:NMAT,1:NMAT),DerivH_pre(1:nsite,1:ndim,1:NMAT,1:NMAT)
  doublecomplex UVd(1:NMAT,1:NMAT)
  doublecomplex UV(1:NMAT,1:NMAT)
  doublecomplex UdVd(1:NMAT,1:NMAT)
  doublecomplex UdV(1:NMAT,1:NMAT)
  doublecomplex UU(1:NMAT,1:NMAT)
  doublecomplex UdUd(1:NMAT,1:NMAT)
  doublecomplex UUd(1:NMAT,1:NMAT)
  doublecomplex UdU(1:NMAT,1:NMAT)
  doublecomplex temp(1:NMAT,1:NMAT)
  ...
  do t=1,nsite
     ...
     do idim=1,ndim   
        ...
        do imat=1,NMAT
           do jmat=1,NMAT
              UVd(imat,jmat)=UVd(imat,jmat)+U(t,idim,imat,jmat)*dconjg(V(jmat))
              UdV(imat,jmat)=UdV(imat,jmat)+dconjg(U(tm1,idim,jmat,imat))*V(jmat)
           end do
        end do
        ...
        do imat=1,NMAT
           do jmat=1,NMAT
              do kmat=1,NMAT
                 DerivH_pre(t,idim,imat,jmat)=DerivH_pre(t,idim,imat,jmat)+UVd(imat,kmat)*UdV(kmat,jmat)
              end do
           end do
        end do
        UdVd=(0d0,0d0)
        ...
        do imat=1,NMAT
           do jmat=1,NMAT
              UdVd(imat,jmat)=UdVd(imat,jmat)+dconjg(U(tp1,idim,jmat,imat))*dconjg(V(jmat))
              UV(imat,jmat)=UV(imat,jmat)+U(t,idim,imat,jmat)*V(jmat)
           end do
        end do
        ...
        do imat=1,NMAT
           do jmat=1,NMAT
              do kmat=1,NMAT
                 DerivH_pre(t,idim,imat,jmat)=DerivH_pre(t,idim,imat,jmat)+UV(imat,kmat)*UdVd(kmat,jmat)
              end do
           end do
        end do
     end do
