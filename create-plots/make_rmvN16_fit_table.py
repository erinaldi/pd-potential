...
def get_fit_coeffs_SameType(pointset):
    Ls = pointset.Ls
    USE_POINTS = [0,1,2,3]

    gW = gvar.gvar(pointset.WL_continuum_largeN_together, pointset.WL_cln_together_err)
    y_for_fit = np.log(gW[USE_POINTS])
    (c, d), cov = np.polyfit(Ls[USE_POINTS], gvar.mean(y_for_fit), w=1/gvar.sdev(y_for_fit), deg=1, cov='unscaled')
...
def main():
    ...
    for rmvN in remove_Ns:
        ...
        c_con_only_p02, _ = get_fit_coeffs_SameType(WL_con_P020)
        c_con_only_p025, _ =  get_fit_coeffs_SameType(WL_con_P025)
    ...
    header = ["P"] + [f"rmv. $N={rmvN}$" if rmvN is not None else "all $N$" for rmvN in remove_Ns]
    with open(Path("products", "tables", "c_con_only_removeN.csv"), "w+") as f:
        writer = csv.writer(f)
        writer.writerow(header)
        p02_line = ["$0.2$"] + [c_cons_oneType_p02[rmvN] for rmvN in remove_Ns]
        p025_line = ["$0.25$"] + [c_cons_oneType_p025[rmvN] for rmvN in remove_Ns]
        writer.writerow(p02_line)
        writer.writerow(p025_line)
    ...
    # FOR REMOVING n_t=16
    # SORRY FOR IN-LINING AND REPEATING....
    T = 0.29
    points:List[Point] = []
    w_types = ("con", "mix")
    nmp = ((16,8,0.2), (16,10,0.25), (24,11,0.2), (24,15,0.25), (32,15,0.2), (32,20,0.25), (64,29,0.2), (64,39,0.25)) 
    ...
    header = ["P"] + [f"rmv. $N={rmvN}$" if rmvN is not None else "all $N$" for rmvN in remove_Ns]
    with open(Path("products", "tables", "c_con_only_removeN_removed_nt16.csv"), "w+") as f:
        writer = csv.writer(f)
        writer.writerow(header)
        p02_line = ["$0.2$"] + [c_cons_oneType_p02[rmvN] for rmvN in remove_Ns]
        p025_line = ["$0.25$"] + [c_cons_oneType_p025[rmvN] for rmvN in remove_Ns]
